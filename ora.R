...
res_entrez %<>% filter(!is.na(ENTREZID) & !duplicated(ENTREZID))

#' We can check which pathways are enriched in our data using the `enrichKEGG`
#' function
kegg_res <- enrichKEGG(filter(res_entrez, !is.na(padj) & padj < 0.05)$ENTREZID)
head(kegg_res)

#' If we want detailed info on a pathway, we can make use of the pathview
#' package. Let's zoom in on the MAPK (hsa04010) pathway.
...
img <- image_read(here("hsa04010.pathview.png"))
...
mkegg_res <- enrichMKEGG(filter(res_entrez, !is.na(padj) & padj < 0.05)$ENTREZID)
head(mkegg_res)

#' For disease focused research, one sometime wants to explore disease
#' ontology
do_res <- enrichDO(filter(res_entrez, !is.na(padj) & padj < 0.05)$ENTREZID)
head(do_res)
...
pw_res <- enrichPathway(filter(res_entrez, !is.na(padj) & padj < 0.05)$ENTREZID)
head(pw_res)

#+ fig.width=10, fig.height=6
viewPathway("TP53 Regulates Transcription of Cell Death Genes", 
            readable = TRUE, 
